...
### 每日整理（2026-09-14 18:00 · 自动化触发）
- 按 CLAUDE.md **第八节**「每日任务」流程执行（用户消息写「第 9 节」，实为第八节；第九节是每周体检）。调度 ID = `13590c9d-5273-4345-a929-0ea323ce079d`。
- `Clippings/` 已存在但**为空**，无剪藏需归类清空（与 09-11 起连续第 4 日空）。
- `raw/` 今日无新增/修改；`raw/珍大户/` 6 文件 mtime 维持 2026-09-02。`mine/` 不存在。
- `output/` 今日无新增问答/报告/幻灯片等沉淀对象（最新文件为 2026-09-13 daily-log）。
- **结论：当日无新资料进入 raw/mine/output，不触发 raw→wiki 编译，也不触发 output→wiki/skills 沉淀。**
- 复核 `wiki/index.md`：统计与 09-11 校正基线（活跃 84 页）一致，无需改动。
- 轻量链接检查（`.workbuddy/linkcheck.py`，91 全页含备份 + moc 占位）：
  - 断链全部为已知历史/结构性项（`[[CLAUDE]]` 19 处、`log.md` 历史旧称约 13 处、`[[...]]`/`[[双链]]` 示例各 2–3 处、珍大户图片嵌入约 48 处），**无新增断链**。
  - 孤儿页：`wiki/log.md` + `wiki/moc/我的风格.md`（均符合预期，无入链需求）。
- 覆写 `output/daily-log-2026-09-14.md`（当日无变更）。
- 合规自检：未修改 `raw/`、`mine/`（不存在）任何文件；仅追加本段并覆写 daily-log。✅

## 2026-09-15

### 每日整理（2026-09-15 18:00 · 自动化触发）
- 按 CLAUDE.md **第八节**「每日任务」流程执行（用户消息写「第 9 节」，实为第八节；第九节是每周体检）。调度 ID = `13590c9d-5273-4345-a929-0ea323ce079d`。
- `Clippings/` 已存在但**为空**，无剪藏需归类清空（与 09-11 起连续第 5 日空）。
- `raw/` 今日无新增/修改；`raw/珍大户/` 6 文件 mtime 维持 2026-09-02。`mine/` 不存在。
- `output/` 今日无新增问答/报告/幻灯片等沉淀对象（最新文件为 2026-09-14 daily-log）。
- 全库今日（2026-09-15）新增/修改扫描（排除 `.workbuddy/`、`_tmp/`、缓存与日志）：仅 `.git/FETCH_HEAD` 与 `.obsidian/plugins/copilot/data.json`（均非知识库内容），**无内容性新料**。
- **结论：当日无新资料进入 raw/mine/output，不触发 raw→wiki 编译，也不触发 output→wiki/skills 沉淀。**
- 复核 `wiki/index.md`：统计与 09-11 校正基线（活跃 84 页）一致，无需改动。
- 轻量链接检查（`.workbuddy/linkcheck.py`，91 全页含备份 + moc 占位）：
  - 断链全部为已知历史/结构性项（`[[CLAUDE]]` 20 处、`log.md` 历史旧称约 13 处、`[[...]]`/`[[双链]]` 示例各 3–4 处、珍大户图片嵌入约 48 处），**无新增断链**。
  - 孤儿页：`wiki/log.md` + `wiki/moc/我的风格.md`（均符合预期，无入链需求）。
- 覆写 `output/daily-log-2026-09-15.md`（当日无变更）。
- 合规自检：未修改 `raw/`、`mine/`（不存在）任何文件；仅追加本段并覆写 daily-log。✅

## 2026-09-16

### 每日整理（2026-09-16 18:00 · 自动化触发）
- 按 CLAUDE.md **第八节**「每日任务」流程执行（用户消息写「第 9 节」，实为第八节；第九节是每周体检）。调度 ID = `13590c9d-5273-4345-a929-0ea323ce079d`。
- `Clippings/` 已存在但**为空**，无剪藏需归类清空（与 09-11 起连续第 6 日空）。
- `raw/` 今日无新增/修改；`raw/珍大户/` 6 文件 mtime 维持 2026-09-02。`mine/` 不存在。
- `output/` 今日无新增问答/报告/幻灯片等沉淀对象（最新文件为 2026-09-15 daily-log）。
- 全库今日（2026-09-16）新增/修改扫描（排除 `.workbuddy/`、`.git/`、`_tmp/`、缓存与日志）：仅 `.obsidian/plugins/copilot/data.json` 与 `.obsidian/workspace.json`（均非知识库内容），**无内容性新料**。
- **结论：当日无新资料进入 raw/mine/output，不触发 raw→wiki 编译，也不触发 output→wiki/skills 沉淀。**
- 复核 `wiki/index.md`：统计与 09-11 校正基线（活跃 84 页）一致，无需改动。
- 轻量链接检查（`.workbuddy/linkcheck.py`，91 全页含备份 + moc 占位）：
  - 断链全部为已知历史/结构性项（`[[CLAUDE]]` 21 处、`log.md` 历史旧称约 20 处、`[[...]]`/`[[双链]]` 示例各 4–5 处、珍大户图片嵌入约 48 处），**无新增断链**。
  - 孤儿页：`wiki/log.md` + `wiki/moc/我的风格.md`（均符合预期，无入链需求）。
- 覆写 `output/daily-log-2026-09-16.md`（当日无变更）。
- 合规自检：未修改 `raw/`、`mine/`（不存在）任何文件；仅追加本段并覆写 daily-log。✅

## 2026-09-17

### 新增 Docker 完整学习路径课程（用户主动请求）
- 用户在根目录新建 `docker/` 文件夹，要求放一份「详细、完整学习路径」的 Docker 教程课程。
- 沿用知识库 wiki 标准格式（frontmatter + 摘要 + 正文 + 相关 + 来源），全部 16 篇用 `[[双链]]` 互链，按「认知→基础操作→构建→连通→编排→生产」四阶段编排：
  - `00-学习路径总览.md`（MOC 总索引，含四阶段路线图与跳转表）
  - 阶段一 认知与准备：`01-容器与Docker入门`、`02-安装与环境配置`、`03-镜像容器仓库核心概念`
  - 阶段二 日常操作：`04-镜像操作实战`、`05-容器生命周期管理`、`06-容器内操作与调试`
  - 阶段三 构建与连通：`07-Dockerfile完全指南`、`08-构建你的第一个镜像`、`09-数据持久化Volume与挂载`、`10-容器网络`
  - 阶段四 编排与生产：`11-Docker-Compose编排`、`12-实战全栈应用编排`、`13-Docker-Hub与私有仓库`、`14-生产环境最佳实践`、`15-常见问题与排错`
- 内容基线：Docker Engine 27.x / Compose v2；覆盖 Windows(WSL2)/Mac/Linux 安装、国内镜像加速器、多阶段构建、web+api+postgres+redis 四服务 Compose 实战、Harbor/Trivy 企业级仓库、生产 checklist。
- 链接校验：16 篇之间 `[[双链]]` 全部指向已存在文件，**0 悬空链接**。
- 联动更新：`wiki/index.md`（主题数 5→6，新增 Docker 课程指针；总页面统计补充说明 `docker/` 16 篇未计入 wiki 页面数）、本日志。
- 合规自检：仅新增 `docker/` 16 篇 + 更新 `wiki/index.md` 与 `wiki/log.md`；未改动 `raw/`、`mine/`（不存在）。✅

### 路径校正（当日补充）
- 用户指出 Docker 课程不应放在 vault 根目录 `docker/`，而应按 CLAUDE.md 的 [[CLAUDE]] 目录规范归入 `wiki/<主题>/` 子目录，且主题索引须命名为 `主题-index.md`（对齐 `git-index`/`云原生-index` 等）。
- 已执行迁移：`E:/999知识库/docker/` → `E:/999知识库/wiki/docker/`；将总览页 `00-学习路径总览.md` 重命名为 `docker-index.md`，frontmatter title 改为「Docker 主题索引（学习路径总览）」。
- 唯一跨目录双链修正：`wiki/index.md` 中 `[[00-学习路径总览]]` → `[[docker-index]]`，路径说明 `docker/` → `wiki/docker/`。docker 16 篇之间 `[[双链]]` 均为 basename，文件夹移动不改变文件名，故内部链接全部保持有效。
- 现状：`wiki/docker/` 含 `docker-index.md` + 15 篇课程，符合 vault 主题子目录规范，0 悬空链接。✅

### 每日整理（2026-09-17 18:00 · 自动化触发）
- 按 CLAUDE.md **第八节**「每日任务」流程执行（用户消息写「第 9 节」，实为第八节；第九节是每周体检）。调度 ID = `13590c9d-5273-4345-a929-0ea323ce079d`。
- `Clippings/` 已存在但**为空**，无剪藏需归类清空（与 09-11 起连续第 7 日空）。
- `raw/` 今日无新增/修改；`raw/珍大户/` 6 文件 mtime 维持 2026-09-02。`mine/` 不存在。
- `output/` 今日无新增问答/报告/幻灯片等沉淀对象（最新文件为 2026-09-16 daily-log）。
- 当日用户主动新增 **Docker 完整学习路径课程**（`wiki/docker/` 16 篇 + `docker-index.md`），已由用户本人完成格式、双链与路径校正并写入 `wiki/index.md` / `wiki/log.md`。该内容非来自 raw/output，故不触发「raw→wiki 编译」与「output→wiki/skills 沉淀」；但因其影响总索引统计，已校正 `wiki/index.md`：
  - 活跃 wiki 页面数 84 → **101**（补计移入 `wiki/docker/` 的 Docker 16 篇 + 09-11 健康检查发现新增的占位页 `moc/我的风格.md` 1 篇；不含 `珍大户/_旧版备份` 17 页）。
  - 移除「独立课程 docker/ 16 篇（未计入）」的过时表述；frontmatter `updated` bump 至 2026-09-17。
  - 跨主题补链：`云原生-index` 相关区新增 `[[docker-index]]`，强化容器技术主题网络。
- 轻量链接检查（`.workbuddy/linkcheck.py`，101 活跃页 / 107 全页含备份）：
  - 断链全部为已知历史/结构性项（`[[CLAUDE]]` 23 处、log.md 历史旧称约 20 处、珍大户图片嵌入约 48 处、`[[双链]]`/`[[...]]` 示例各数处），**Docker 主题 0 悬空链接，无新增断链**。
  - 孤儿页：`wiki/log.md` + `wiki/moc/我的风格.md`（均符合预期，无入链需求）。
- 产出 `output/daily-log-2026-09-17.md`。
- 合规自检：未修改 `raw/`、`mine/`（不存在）任何文件；仅校正 `wiki/index.md` 统计、补充跨主题双链、追加本段。✅

## [2026-09-18] 每周体检（自动化触发 · CLAUDE.md 第 9 节细化为 8 步）

- 触发：自动化调度，按 CLAUDE.md 第 9 节「每周体检」执行（用户将第 9 节细化为 8 步流程）。
- 体检范围：`wiki/` 全量 118 文件（活跃 **101** ＋ `珍大户/_旧版备份` 17），6 大主题 ＋ `moc/` ＋ 根级 index/log。
- 结果摘要：
  - **孤儿页：0**（活跃页均无入链缺失；`log.md`/`moc/我的风格.md` 为符合预期的元/占位页，非内容孤儿）。
  - **长期 stub（停留 > 2 周）：0**（全库页面均为实质内容，占位结构判定排除 log.md「待补」误标）。
  - **断链：无可直接修正项**。分类——`[[CLAUDE]]` 结构性有效 ×7；`log.md` 历史旧称（append-only 保留）×9；唯一遗留 `docker/docker-index.md` 中 `[[双链]]` 1 处（元语言表述，无改名目标，已列入报告待人工决策：改纯文本或新建 `双链.md`）。图片嵌入 `![[...]]` 不计入断链。
  - **矛盾/过时核实（联网复核，已修正）**：发现并修正上周埋下的 1 处事实错误——`01-JDK21新特性.md` 原注「NFTC→OTN 已生效、最后免费更新 2026-07」为误记；Oracle 官方确认 JDK 21 更新在 2026 年 9 月前仍属 NFTC，**切换 OTN 自 2026 年 10 月 CPU 起**，今日（09-18）尚未发生。另更新 `02-Spring4新特性.md`（4.1.1 / 4.2.0-M1 货币性、4.1 OSS 截止对齐 2027-06-30）、`05-升级必要性.md`（切换时点精度）。三篇均补「数据核实时点：2026-09-18」并 bump `updated`。已确认准确：Spring Boot 4.1.0 发布日 2026-06-10、3.5 EOL 2026-06-30。
  - **内容重叠**：3-char shingles 相似度（阈值 0.40）**0 对**达合并阈值，高相似均为刻意姐妹篇/聚合/模板，**无应合并项**。
  - **写作风格提炼**：仍受阻——`mine/` 目录依旧不存在（用户步骤称 `mine/已发布/`、CLAUDE.md §5 称 `mine/已定稿/`，均不存在），无可读取定稿；未读 `mine/草稿/`、未用 `mine/AI生成/`，未编造。`wiki/moc/我的风格.md` 更新状态段。
  - **新条目候选（5）**：OpenJDK 发行版选型与落地（NFTC 到期应对，优先级最高）、Spring Boot 4.2 跟踪、JDK 25/27 落地评估、AI 辅助编程 Skill 本公司落地规范、Spring Boot 4.1 可观测性落地手册。
- 本周净新增条目：**16**（全部 `wiki/docker/` 课程，2026-09-17 由用户本人新增并完成双链/路径校正）；活跃页 84 → 101。
- 产出：`output/health-check-2026-09-18.md`。
- 合规自检：未修改 `raw/`、`mine/`（不存在）任何文件；仅对 3 个 JDK&Spring 条目做「核实注解 ＋ `updated`」、更新 1 个占位页状态，并新增本报告。✅

### 每日整理（2026-09-18 18:12 · 自动化触发）
- 按 CLAUDE.md **第八节**「每日任务」流程执行（用户消息写「第 9 节」，实为第八节；第九节是每周体检）。调度 ID = `13590c9d-5273-4345-a929-0ea323ce079d`。
- `Clippings/` 已存在但**为空**，无剪藏需归类清空（与 09-11 起连续第 8 日空）。
- `raw/` 今日无新增/修改；`raw/珍大户/` 6 文件 mtime 维持 2026-09-02。`mine/` 不存在。
- `output/` 今日新增仅 `health-check-2026-09-18.md`（18:10，由每周体检自动化 `43d342d4` 产出）；判定为**周期性分析报告（一次性产物）**，结论已由该体检任务就地应用进 wiki，**无需进一步沉淀**为 wiki 文章或 skill；留原地。
- **结论：当日无新资料进入 raw/mine，不触发「raw→wiki 编译」，也不触发 output→wiki/skills 沉淀。**
- 复核 `wiki/index.md`：统计与 09-17 校正基线（活跃 **101** 页）一致，无需改动。
- 轻量链接检查（`.workbuddy/linkcheck.py` 全量扫描）：断链全部为已知历史/结构性项（`[[CLAUDE]]` 结构性有效、`log.md` 历史旧称、`docker/docker-index.md` 中 `[[双链]]` 1 处属待人工决策、珍大户 `![[...]]` 图片嵌入约 48 处为有效嵌入误报），**无新增断链**；孤儿页仅 `wiki/log.md` 与 `wiki/moc/我的风格.md`（均符合预期）。
- 覆写 `output/daily-log-2026-09-18.md`（当日无变更）。
- 合规自检：未修改 `raw/`、`mine/`（不存在）任何文件；仅追加本段并覆写 daily-log。✅

### 每日整理（2026-09-19 18:00 · 自动化触发）
- 按 CLAUDE.md **第八节**「每日任务」流程执行（用户消息写「第 9 节」，实为第八节；第九节是每周体检）。调度 ID = `13590c9d-5273-4345-a929-0ea323ce079d`。
- `Clippings/` 已存在但**为空**，无剪藏需归类清空（自 09-11 起连续第 9 日空）。
- `raw/` 今日无新增/修改；`raw/珍大户/` 6 文件 mtime 维持 2026-09-02。`mine/` 不存在。
- `output/` 今日**无新增**问答/报告/幻灯片等沉淀对象（最新为 2026-09-18 两份 daily-log / health-check）；`mine/` 不存在。
- 全库今日（2026-09-19）新增/修改扫描（排除 `.git/`、`.obsidian/`、`.workbuddy/`、`_tmp/`、`copilot/`、`node_modules/`）：**0 个内容性文件**，无任何新增或修改。
- **结论：当日无新资料进入 raw/mine/output，不触发 raw→wiki 编译，也不触发 output→wiki/skills 沉淀。**
- 复核 `wiki/index.md`：统计与 09-17 校正基线（活跃 **101** 页）一致，无需改动。
- 轻量链接检查（`.workbuddy/linkcheck.py` 全量扫描，107 全页含 `珍大户/_旧版备份`）：断链全部为已知历史/结构性项（`[[CLAUDE]]` 结构性有效、`log.md` 历史旧称、`docker-index.md` 中 `[[双链]]` 1 处属待人工决策、珍大户 `![[...]]` 图片嵌入约 48 处为有效嵌入误报），**无新增断链**；孤儿页仅 `wiki/log.md` 与 `wiki/moc/我的风格.md`（均符合预期）。
- 覆写 `output/daily-log-2026-09-19.md`（当日无变更）。
- 合规自检：未修改 `raw/`、`mine/`（不存在）任何文件；仅追加本段并覆写 daily-log。✅
